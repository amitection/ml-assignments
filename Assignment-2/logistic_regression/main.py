...
def plotData(X,y):
  ...
  ax.set_xlabel('Blood Test 1')
  ax.set_ylabel('Blood Test 2')  
...
def computeCost(X, y, theta):
  ...
  pred = X.dot(theta)  
  power = np.negative(y.dot(pred))
  exponent = np.exp(power)
  
  j_theta = np.sum(np.log(1+exponent)) / len(y)
  return j_theta
...
def computeGradient(X,y,theta):
  ...
  pred = X.dot(theta)
  exponent = np.exp( - np.multiply(y, pred))
  
  gradient = np.negative(X.T.dot( np.multiply(y , (exponent / (1+exponent)) ))) 
  gradient = gradient / y.shape[0]
  return gradient
